...
@section('content')
    <div class="max-w-3xl mx-auto p-6">
        ...
        {{-- فرم اضافه کردن --}}
        <form method="POST" action="{{ route('DigitalBrands.store') }}" class="flex gap-2 mb-6">
            @csrf
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            @foreach($brands as $brand)
                <a href="{{route('DigitalBrands.models.index' , $brand)}}">
                <div class="border rounded-xl p-4 flex justify-between items-center bg-gray-50 hover:bg-gray-100 transition">
                    <span>{{ $brand->name }}</span>
                    <form method="POST" action="{{ route('DigitalBrands.destroy', $brand) }}" onsubmit="return confirm('حذف شود؟')">
                        @csrf @method('DELETE')
